...
function getParams() {
    ...
    var count = 0, 
        params = [repeatRate, creditScoreAcc, loanAmt, intRate, cofRate, initialCust, newCustRate];
    for (i=0; i<params.length; i++) { if(isNaN(params[i])) {count = count + 1;}}
    if(count>0) {
        return alert('Try again! The input boxes only accept numbers.');
    }
    else {return params;}
}
